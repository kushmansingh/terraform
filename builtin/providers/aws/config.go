...
func (c *Config) Client() (interface{}, error) {
	...
	if err != nil {
		if awsErr, ok := err.(awserr.Error); ok && awsErr.Code() == "NoCredentialProviders" {
			// If a profile wasn' specifed error out
			if c.Profile == "" {
 ...
 providing credentials for the AWS Provider`)
		...
		// add the validate credentials to the session options
		log.Printf("[INFO] AWS Auth provider used: %q", cp.ProviderName)
		opt.Config.Credentials = creds
	}

	if logging.IsDebugOrHigher() {
		opt.Config.LogLevel = aws.LogLevel(aws.LogDebugWithHTTPBody)
 ...
 providing credentials for the AWS Provider`)
	...
	// if the desire number of retries is non-zero, update the session
	if c.MaxRetries > 0 {
